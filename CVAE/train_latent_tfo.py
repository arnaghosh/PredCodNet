...
def plot_results():
	...
	with torch.no_grad():
		...
		recon_train_images, mu,sigma,latent,latent_hat = network(train_image_tensor.view(-1,28*28),train_rot_tensor,train_trans_tensor)
	...
	with torch.no_grad():
		...
		recon_test_images, mu,sigma,latent,latent_hat = network(test_image_tensor.view(-1,28*28),test_rot_tensor,test_trans_tensor)
	...
	with torch.no_grad():
		for batch_idx, (data, rot, trans, data_tfo, target) in enumerate(test_loader):
			...
			data_tfo = data_tfo.to(device)
			recon, mu, sigma, latent_z, latent_z_hat = network(data.view(-1,28*28),rot,trans)
