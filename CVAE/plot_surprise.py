...
def get_surprise(network, data, rot, trans, data_tfo):
	if type(network).__name__ == 'CVAE_MLP':
		...
		with torch.no_grad():
			x_hat, mu1, sigma1, z = network(data_tfo,rot_zero,trans_zero)
		...
		trans_zero = trans.new_zeros(trans.size())
		with torch.no_grad():
			x_hat, mu1, sigma1, z, z_hat = network(data_tfo,rot_zero,trans_zero)
			x_hat, mu2, sigma2, z, z_hat = network(data,rot,trans)
		...
		trans_zero = trans.new_zeros(trans.size())
		with torch.no_grad():
			x_hat, x_recon, mu1, sigma1, z, z_hat = network(data_tfo,rot_zero,trans_zero)
			x_hat, x_recon, mu2, sigma2, z, z_hat = network(data,rot,trans)
			# z = mu + sigma*torch.randn_like(mu)
			rot = rot.view(-1,1)
			trans = trans.view(-1,1)
			C = torch.cat((rot,trans),dim=-1)
