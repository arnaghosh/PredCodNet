...
run_updated_ver = 1
network = CVAE_MLP_latent_tfo_modif(28*28,[250,100],50,norm_context=1).to(device)
learning_rate = 0.002
...
def train(epoch):
	...
	for batch_idx, (data,rot,trans,data_tfo,target) in enumerate(train_loader):
		...
		recon_tfo, recon, mu, sigma, latent_z, latent_z_hat = network(data,rot,trans,data_tfo)
		...
		if run_updated_ver:
			...
			data_tfo = data_tfo.to(device)
			recon_tfo, recon, mu, sigma, latent_z, latent_z_hat = network(data_tfo,rot_zero,trans_zero,data_tfo)
			loss = VAE_loss_modif(data_tfo,recon,data_tfo,recon_tfo,mu,sigma)
			loss.backward()
			optimizer.step()
	...
	with torch.no_grad():
		for batch_idx, (data,rot,trans,data_tfo,target) in enumerate(test_loader):
			...
			if run_updated_ver:
				...
				data_tfo = data_tfo.to(device)
				recon_tfo, recon, mu, sigma, latent_z, latent_z_hat = network(data_tfo,rot_zero,trans_zero,data_tfo)
				test_loss += VAE_loss_modif(data_tfo,recon,data_tfo,recon_tfo,mu,sigma).cpu().item()
				test_cntr +=1
	test_loss/=test_cntr
...
def plot_results():
	...
	with torch.no_grad():
		...
		recon_train_images_tfo, recon_train_images, mu,sigma,latent,latent_hat = network(train_image_tensor,train_rot_tensor,train_trans_tensor,train_image_tfo_tensor)
	...
	with torch.no_grad():
		...
		recon_test_images_tfo, recon_test_images, mu,sigma,latent,latent_hat = network(test_image_tensor,test_rot_tensor,test_trans_tensor,test_image_tfo_tensor)
