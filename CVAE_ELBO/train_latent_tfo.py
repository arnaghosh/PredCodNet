...
run_updated_ver = 1
network = CVAE_MLP_latent_tfo(28*28,[250,100],50,norm_context=1).to(device)
learning_rate = 0.002
...
def train(epoch):
	...
	for batch_idx, (data,rot,trans,data_tfo,target) in enumerate(train_loader):
		...
		recon, mu, sigma, latent_z, latent_z_hat = network(data,rot,trans,data_tfo)
		...
		if run_updated_ver:
			...
			data_tfo = data_tfo.to(device)
			recon, mu, sigma, latent_z, latent_z_hat = network(data_tfo,rot_zero,trans_zero,data_tfo)
			loss = VAE_loss(data_tfo,recon,mu,sigma)
			loss.backward()
			optimizer.step()
...
def plot_results():
	...
	with torch.no_grad():
		...
		recon_train_images, mu,sigma,latent,latent_hat = network(train_image_tensor,train_rot_tensor,train_trans_tensor,train_image_tfo_tensor)
	...
	with torch.no_grad():
		...
		recon_test_images, mu,sigma,latent,latent_hat = network(test_image_tensor,test_rot_tensor,test_trans_tensor,test_image_tfo_tensor)
