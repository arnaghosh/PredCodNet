...
class Encoder_MLP(nn.Module):
	def __init__(self,input_dim,hidden_dims,output_dim):
		...
		self.readout = nn.Linear(2*self.hidden_dims[len(self.hidden_dims)-1],2*self.output_dim)

	def forward(self, X, X_tfo):
		X = X.view(-1,28*28)
		X_tfo = X_tfo.view(-1,28*28)
		X = F.relu(self.linear1(X))
		for hidden_layer in self.hidden.children():
			X = F.relu(hidden_layer(X))
		X_tfo = F.relu(self.linear1(X_tfo))
		for hidden_layer in self.hidden.children():
			X_tfo = F.relu(hidden_layer(X_tfo))
		X1 = torch.cat((X,X_tfo),dim=-1)
		X1 = self.readout(X1)
		mean = X1[:,:self.output_dim]
		# stdev = 1e-6 + F.softplus(X[:,self.output_dim:])
		stdev = 1e-6 + torch.exp(0.5*X1[:,self.output_dim:])
		return mean, stdev

class Encoder_MLP_conditional(nn.Module):
	def __init__(self,input_dim,hidden_dims,output_dim):
		super(Encoder_MLP_conditional, self).__init__()
		self.input_dim = input_dim	# adding 2 for rotation and translation info
		self.hidden_dims = hidden_dims
		self.output_dim = output_dim
		self.linear1 = nn.Linear(self.input_dim,self.hidden_dims[0])
		self.hidden = [nn.Linear(self.hidden_dims[i-1],self.hidden_dims[i]) for i in range(1,len(self.hidden_dims))]
		self.hidden = nn.Sequential(*self.hidden)
		self.readout = nn.Linear(2+2*self.hidden_dims[len(self.hidden_dims)-1],2*self.output_dim)

	def forward(self, X, c1, c2, X_tfo):
		X = X.view(-1,28*28)
		X_tfo = X_tfo.view(-1,28*28)
		c1 = c1.view(-1,1)
		c2 = c2.view(-1,1)
		...
		for hidden_layer in self.hidden.children():
			X_tfo = F.relu(hidden_layer(X_tfo))
		X1 = torch.cat((X,X_tfo,c1,c2),dim=-1)
		X1 = self.readout(X1)
		mean = X1[:,:self.output_dim]
		# stdev = 1e-6 + F.softplus(X[:,self.output_dim:])
		stdev = 1e-6 + torch.exp(0.5*X1[:,self.output_dim:])
...
class CVAE_MLP(nn.Module):
	...
	def forward(self, X, c1, c2, X_tfo):
		if self.enc_condition:
			mu,sigma = self.encoder(X,c1,c2, X_tfo)
		else:
			mu,sigma = self.encoder(X, X_tfo)
		z = mu + sigma*torch.randn_like(mu)
		if self.dec_condition:
...
class CVAE_MLP_latent_tfo(nn.Module):
	def __init__(self, input_dim, hidden_dims, output_dim, norm_context=1):
	...
	def forward(self, X, c1, c2, X_tfo):
		...
		if self.norm_context:
			c1_norm = c1/self.max_rotation
			c2_norm = c2/self.max_translation
		else:
			c1_norm = c1
			c2_norm = c2
		C = torch.cat((c1_norm,c2_norm),dim=-1)
		mu,sigma = self.encoder(X,c1,c2,X_tfo)
		z = mu + sigma*torch.randn_like(mu)
